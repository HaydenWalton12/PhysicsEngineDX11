...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 frame_size)
{
    ...
    if (values_count >= values_count_min)
    {
        ...
        for (int n = 0; n < res_w; n++)
        {
            ...
            ImVec2 pos_1 = ImLerp(inner_bb.Min, inner_bb.Max, (plot_type == ImGuiPlotType_Lines) ? tp1 : ImVec2(tp1.x, histogram_zero_line_t));
            if (plot_type == ImGuiPlotType_Lines)
            {
                window->DrawList->AddLine(pos0, pos_1, idx_hovered == v1_idx ? col_hovered : col_base);
            }
            else if (plot_type == ImGuiPlotType_Histogram)
            {
                if (pos_1.x >= pos0.x + 2.0f)
                    pos_1.x -= 1.0f;
                window->DrawList->AddRectFilled(pos0, pos_1, idx_hovered == v1_idx ? col_hovered : col_base);
            }
